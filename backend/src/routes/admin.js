...
export async function handleAdmin(request, env, path) {
  ...
  if (path === '/api/admin/add-client' && request.method === 'POST') {
    ...
    const { businessName, businessEmail, widgetKey, description, plan, websiteUrl } = body;
    if (!businessName || !businessEmail || !widgetKey) {
    ...
    await env.DB.prepare(`
      INSERT INTO clients (business_name, business_email, widget_key, description, plan, website_url, active, created_at)
      VALUES (?, ?, ?, ?, ?, ?, 1, ?)
    `).bind(
      businessName, businessEmail, widgetKey,
